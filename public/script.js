...
function handleIncomingPixel(p){
  ...
  ctx.fillStyle=p.color; ctx.fillRect(p.x,p.y,GRID_SIZE,GRID_SIZE); ctx.restore();
  playSound(drawAudio);
}
...
function updatePointsDisplay(){
  ...
  else{ const now=Date.now(); const timeLeft=Math.max(0,Math.ceil((30000-(now-lastActionTime))/1000));
