...
document.addEventListener("click", e => {
  if (!moreColorsPopup.contains(e.target) && e.target !== moreBtn) moreColorsPopup.classList.remove("show");
});
...
function updatePoints(p){points=p;pointsDisplay.textContent=`${points}/${MAX_POINTS}`;updateRing();}
setInterval(()=>{
  const now=Date.now();
  if(points<MAX_POINTS && now-lastPlaceTime>=COOLDOWN){points++;updatePoints(points);lastPlaceTime=now;}
},1000);
...
canvas.addEventListener("click",e=>{
  ...
  if(soundEnabled) drawAudio.cloneNode().play();
  points=Math.max(0,points-1);lastPlaceTime=Date.now();updatePoints(points);
});
...
function drawGrid(){
  ...
  if(showGrid){ctx.strokeStyle="#ddd";ctx.lineWidth=1/scale;for(let x=0;x<=WORLD_WIDTH;x+=GRID_SIZE){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,WORLD_HEIGHT);ctx.stroke();}
